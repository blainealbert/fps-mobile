...
namespace Unity.FPS.Gameplay
{
    public class PlayerInputHandler : MonoBehaviour
    {
        ...
        private bool UIJumpButtonPressed = false;
        private bool UIFireButtonPressed = false;
        private bool UIAimButtonPressed = false;
        ...
        public void UIJumpButtonSetActive() {
            Debug.Log("Jump");
            UIJumpButtonPressed = true;
        }

        public void UIJumpButtonSetInactive() {
            Debug.Log("Stop Jump");
            UIJumpButtonPressed = false;
        }

        public void UIFireButtonSetActive() {
            Debug.Log("Firing");
            UIFireButtonPressed = true;
        }

        public void UIFireButtonSetInactive() {
            Debug.Log("Stop Firing");
            UIFireButtonPressed = false;
        }

        public void UIAimButtonToggle() {
            Debug.Log("Toggle Aim");
            UIAimButtonPressed = !UIAimButtonPressed;
        }
        ...
        public Vector3 GetMoveInput()
        {
            if (CanProcessInput())
            {
                // old button movement input
                // Vector3 move = new Vector3(Input.GetAxisRaw(GameConstants.k_AxisNameHorizontal), 0f,
                //     Input.GetAxisRaw(GameConstants.k_AxisNameVertical));

                // get joystick input movement
                Vector3 move = new Vector3(GetComponent<MobileJoystickPlayerController>().horizontalInput, 0f,
                    GetComponent<MobileJoystickPlayerController>().verticalInput);
        ...
        {
            // check for mobile right mobile joystick value
            // if(rightJoystick.GetComponent<MobileJoystick>().Horizontal > 0) {
                return (rightJoystick.GetComponent<MobileJoystick>().Horizontal / 25);
            // }
        ...
        {
            // check for mobile right mobile joystick value
            // if(rightJoystick.GetComponent<MobileJoystick>().Vertical > 0) {
                return (rightJoystick.GetComponent<MobileJoystick>().Vertical / 75) * -1;
            // }
        ...
        public bool GetAimInputHeld()
        {
            if (CanProcessInput())
            {
                if(UIAimButtonPressed) {
                    return true;
                }
                // commenting out right click since it is picked up by multipress gesture
